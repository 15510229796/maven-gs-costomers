package com.jk.gs.controller.login.readDat;
...
public class IPSeeker {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = IPProcessUtils.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = IPProcessUtils.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		...
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IPProcessUtils.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(IPProcessUtils.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(IPProcessUtils.getIpByteArrayFromString(ip));
	}
	...
	private long locateIP(byte[] ip) {
		...
		for (long i = ipBegin, j = ipEnd; i < j;) {
			...
			readIP(m, b4);
			r = compareIP(ip, b4);
			// log.debug(IPProcessUtils.getIpStringFromBytes(b));
			if (r > 0)
				i = m;
			else if (r < 0) {
				if (m == j) {
	...
	private String readString(long offset) {
		try {
			...
			for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile
					.readByte())
				;
			if (i != 0) return IPProcessUtils.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
			logger.error(e.getMessage());
		}
		return "";
